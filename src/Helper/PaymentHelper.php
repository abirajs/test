...
class PaymentHelper
{
    ...
    public function createPlentyPayment($paymentResponseData)
    {
        try {
            ...
            $payment->amount          = ($paymentResponseData['result']['status'] == 'SUCCESS' && empty($paymentResponseData['instalment']['cancel_type'])   && !empty($paymentResponseData['transaction']['refund']['amount'])) ? ($paymentResponseData['transaction']['refund']['amount'] / 100) : (($paymentResponseData['instalment']['cycle_amount']) ? (($paymentResponseData['instalment']['cycle_amount']) / 100) : ($paymentResponseData['transaction']['status'] == 'CONFIRMED' ? ($paymentResponseData['transaction']['amount'] / 100) : 0));
